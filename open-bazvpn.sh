...
bazvpn="https://raw.githubusercontent.com/surinvpn/baz-vpn/master"
...
# IP Address
SERVER_IP=$(wget -4qO- "http://whatismyip.akamai.com/")
if [[ "$SERVER_IP" = "" ]]; then
...
cd /
wget -q -O ovpn.tar "$bazvpn/conf/openvpn.tar"
tar xf ovpn.tar
rm ovpn.tar
...
wget -q -O /etc/nginx/nginx.conf "$bazvpn/conf/nginx.conf"
wget -q -O /etc/nginx/conf.d/vps.conf "$bazvpn/conf/vps.conf"
mkdir -p /home/vps/public_html/open-on
wget -q -O /home/vps/public_html/open-on/index.php "$bazvpn/conf/api.txt"
wget -q -O /home/vps/public_html/index.php "$bazvpn/conf/baz-vpn.txt"
...
# download script
cd /usr/bin
wget -q -O menu "$bazvpn/news.sh"
wget -q -O chek "$bazvpn/lock.sh"
wget -q -O m "$bazvpn/menu.sh"
wget -q -O speedtest "$bazvpn/speedtest.py"
wget -q -O b-user "$bazvpn/b-user.sh"
